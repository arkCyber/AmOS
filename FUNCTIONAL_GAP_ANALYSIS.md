...
| **Radio / connectivity（wifi/蓝牙/飞行）** | ✅(内核/桥/UI) | 2026-09-04：`amos-radio` 领域内核（`RadioManager` 策略 + 飞行级联 + `MockRadioProvider`）+ Tauri `radio_*` 命令桥（进程内，真机属 Android 系统服务故不走 daemon）+ 通知中心接线（持久化镜像、跨窗口广播、未 bridge `flipRadio` 降级、开面板 `radioStatus` 权威同步）。真机 Android provider（`AndroidRadioProvider`，`android` feature JNI 骨架）待设备（见 `docs/radio.md`）。**2026-09-14（REQ-A199，真机验收）**：蓝牙详情 —— 适配器名读写（`getName`/`setName`，成功回读权威值）+ 已配对设备读取（`getBondedDevices`）；设备上读出 `A17Pro`、`[]`，改名在 `settings get secure bluetooth_name` 上可见并已还原。**平台限制（javap 取证，非待办）**：「可被发现」无公开 API（`setScanMode` 不在 SDK；只有需 Activity 的 `ACTION_REQUEST_DISCOVERABLE`），「取消配对」无公开 API（`removeBond` 不在 SDK）—— 已在 UI 与 `docs/radio.md` §10 写明。**2026-09-14（REQ-A200，真机验收）**：搜索附近设备 + 发起配对已补全 —— Kotlin `BluetoothGlue`（`BroadcastReceiver` + `startDiscovery`/`bond`）+ `BLUETOOTH_SCAN` 运行时权限 + 四命令 + 搜索 UI（离开页面取消扫描）；真机取证：搜索 → `dumpsys Discovering: true`、UI 渲染 8 行（含 `midea`）、停止/离开均回 `Discovering: false`；`bluetooth_pair` 对无设备地址返回 `true` 而已配对列表仍为空（**"已发起"≠"已配对"**），非法/空地址如实拒绝。**2026-09-14（REQ-A201，真机验收）**：BLE（`BluetoothLeScanner`）搜索 + 配对进度可见 —— 搜索同时走经典发现与 LE（权限仍是同一枚 `BLUETOOTH_SCAN`，增量 0），30 s LE 窗口；`bonded` 布尔升级为 raw `bond` 三态（10/11/12，旧注释里的 11/12/13 已修正）并把每次 `ACTION_BOND_STATE_CHANGED` 记为会话级 `bonds`（含不在结果表里的地址），真机实测 `10→11→10` 全程可见。**2026-09-14（REQ-A202，真机验收）**：射频开关的**平台托管**成为一等事实 —— `RadioProvider::control()` + `RadioControl`/`SystemSurface`/`PlatformReason`，`RadioManager` 在触达平台前拒绝（飞行级联成员任一被托管即整体拒绝，不再半应用），新命令 `radio_control`/`radio_open_settings` 让 UI 说明原因并打开系统设置面板（真机：wifi→`wifi_panel`、bluetooth→`bluetooth_settings`、airplane→`airplane_settings`，权限增量 0）。**2026-09-14（REQ-A203，真机验收）**：`radio_set` 的拒绝成为结构化数据 —— 桥改 `Result<RadioSetPayload,String>`，拒绝 = `Ok(applied:false + RadioRefusalPayload{kind,radio,surface,reason,detail})`（稳定机器 token，拒绝后重读权威状态，`Err` 只剩未知 key），前端 `radioRefusalView()` 压成 `{noteKey,surface}`（`platform_managed` 用被点名成员的句子 + surface），通知中心 tile 与设置页无线局域网/热点三处显示拒绝句（`platform_managed` 附「打开系统设置」，未知 kind 仍说「设备没有说明原因」）；Rust +4 测试、`radioControl` +8、svelte 假桥换结构化应答 +3 条 DOM 断言，全门禁绿；**真机取证（YY000286）**：设置→个人热点（飞行位先经 adb 应用外置位使 provider 真的拒绝）点开关 ⇒ 拒绝句「设备没有接受这次开关操作，状态未改变…」上屏（`provider_refused`→`radio.refused.device`，testid `hotspot-refused`，开关保持关 = 状态来自拒绝后重读），桥级 `airplane`⇒`platform_managed{airplane_settings,privileged_only}`、`hotspot`⇒`provider_refused` 均为结构化应答；`airplane_active` 守卫设备上**结构性不可达**（AmOS 自有飞行位普通安装永远为假，见 `docs/radio.md` §14.2）。**仍缺**：完成一次真实配对（需对端确认，且 `removeBond` 不可用，刻意未对真实设备发起）、配对之后的连接/播放/取消配对 |
...
33. ~~**无周期性健康检查/探针**（`GetStatus` 仅 boot 时查一次）~~ → **✅ 已完成（2026-09-11）**：daemon 侧周期自健康心跳（heartbeat 计数 + 指标日志）随 `serve()` 启停；**客户端侧定时探针**也已在 AI 设置页落地——页面打开且**可见**时每 10 秒重读 `get_status`（驱动 `describeEngine` 的引擎/模型/降级/池/缓存读数），并显示「上次探测 HH:MM:SS」，因此守护进程掉线/降级/换引擎会即时反映而不再留着过期快照；隐藏标签页**不**轮询（不是后台探针）。**（补充 2026-09-14，REQ-A204）同一探针模式补进「关于本机」页**：该页电量原本挂载后只读一次（与 A79 同类的陈旧读），现同样每 10 秒重探 `system_health`/宿主电池、掉线应答把读数重置为诚实的「—」并刷新「上次探测」戳；不可见标签页暂停。
