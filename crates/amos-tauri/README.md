...
| `src/link.rs` | the robot-link bridge: reads the daemon's AmOS-Link control plane (`RobotLink.GetStatus` + `ListActuations`) for the Settings「机器人链路」page — including what each robot reports about its own actuation |
