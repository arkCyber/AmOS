...
3. **设备侧绑定（REQ-A369 已接线）**：`crates/amos-tauri/src/alarm_sched.rs` 的 `#[cfg(feature = "android")] mod android` 持有 `JavaVM` + `Context` 的 `GlobalRef`（由 Kotlin `AlarmGlue.bind` 在 `MainActivity.onStart` 上呼 `Java_com_amos_ai_glue_AlarmGlue_attachContext` 交进来），两个命令各自调用 `AlarmGlue.schedule(ctx, id, atMs)` / `AlarmGlue.cancel(ctx, id)`（`setExactAndAllowWhileIdle(RTC_WAKEUP, …)`）。到点由 `AlarmReceiver`（`AndroidManifest.components.xml`）拉起 System UI 前台，WebView 既有的 `osAlarmWatcher` → `syncDueAlarmAlerts` 显示响铃动画。
   ...
   - **回包是事实，不是空值**：`scheduler_alarm_register` 现在答 `AlarmArmed { id, atMs, device }`，`device` 是类型化的 `DeviceOutcome`（`scheduled` / `disallowed` / `unavailable` / `denied` / `unattached` / `host_only` / `unknown(词)`）；`scheduler_alarm_cancel` 答 `AlarmCanceled { id, wasRegistered, device }`。**只有 `scheduled` 意味着锁屏/息屏后手机真会被叫醒** ✓，其余状态由 Clock 闹钟页的横幅如实显示（`data-testid="alarm-native-wake"`），并在 `disallowed` 时给出出路（`scheduler_alarm_open_settings` → 系统「闹钟和提醒」页 ✓）。
   - **身份规则（生产级，REQ-A373 / F-TAU-011）**：一个闹钟的 `PendingIntent` 身份 = `(action, data=amos-alarm://<id>, class, requestCode)`。**只用 requestCode 不行** —— `PendingIntent` 的相等性**忽略 extras**，而 requestCode 是 `id.hashCode()` ⇒ 两个"只是 hash 相撞"的 id（`"Aa"`/`"BB"` 是教科书例子）会塌成同一个 PendingIntent ⇒ 第二个闹钟覆盖第一个的 id、取消任一个等于取消两个 ✗。故 `AlarmGlue.pending` 额外 `setData(...)`；该规则是**纯函数** `alarmIdentity(id)` ✓，由主机 JVM 测试 `AlarmIdentityTest` 钉住（含同 hash 对必须**不同** ✓），并做过**负控**（把它改成常量 ⇒ 3/3 失败、BUILD FAILED ✓）。
   - **响铃那一刻的边界（生产路径已实施，真机未验）**：`AlarmReceiver` 会"请"System UI 到前台 ✓，但**广播接收器属于后台**，而 Android 10+ 禁止后台应用启动 Activity ⇒ 平台会**丢弃并只留一条日志** ✗（本仓在 API 34 上已为「来电筛选角色对话框」实测过同一规则 ✓）。因此 REQ-A375 落上了**平台许可的那条路**：`AlarmGlue.notifyAlarm` 发一条**全屏 Intent 通知**（`NotificationCompat.Builder.setFullScreenIntent` + `IMPORTANCE_HIGH` 通道 ✓，`setCategory(CATEGORY_ALARM)` ✓），它能在一应用退到后台时把响铃带到用户眼前 ✓。设计上是**加法** ✓：不替换"请前台"这次尝试 ✓、也不替换 WebView 的响铃 ✓ ⇒ 失败只会是"没有可见的响铃" ✓，**绝不会是"没有闹钟"** ✓。它需要 `USE_FULL_SCREEN_INTENT`（安装期 ✓，闹钟/通话类应用 ✓）与 **`POST_NOTIFICATIONS`（API 33+ 运行时 ✓，新装默认没有 ✓）** ⇒ 由 `PermissionWire.notificationsIfNeeded` 在启动时申请 ✓；被拒时 `logcat` 记 `notify-denied` ✓ 而闹钟本身照常响 ✓。
...
5. **身份规则（REQ-A373）**：两个 id 只差 hash 相撞（用 `"Aa"`/`"BB"` 做前缀构造）的两个闹钟先后登记 ⇒ `dumpsys alarm` 里应看到**两条**（而不是互相覆盖 ✓），取消其中一条 ⇒ 另一条仍在 ✓。
6. **后台启动限制（F-TAU-012，本轮新增的必测项）**：`adb shell am force-stop com.amos.ai` + 灭屏之后等到点 ⇒ 看 `logcat` 是否有 `Background activity start ... blocked` ✗ —— 有 ⇒ "顶到最前"这条路在本 ROM 上被平台挡住 ✓（正是全屏 Intent 通知要解决的问题 ✓）；**同一条里还要看**：该时刻是否出现**全屏响铃通知** ✓（`logcat` 里 `AlarmGlue: posted the firing notification …` ✓；若被拒则是 `alarm … its notification was not posted: notify-denied` ✗ ⇒ 先看 `POST_NOTIFICATIONS` 是否已授予 ✓），以及**进程是否被拉起且在 WebView 起来后响铃** ✓ —— **三条分开记** ✓，不要合并成"闹钟响了没有" ✓。
7. **授权出路（REQ-A373）**：`adb shell cmd appops set com.amos.ai SCHEDULE_EXACT_ALARM deny` ⇒ Clock 页横幅出现且带「去授权」按钮 ⇒ 点它应打开系统「闹钟和提醒」页 ✓（`logcat` 里 `AlarmGlue` 无 `could not open` 警告 ✓）；`cmd appops ... allow` 复原。
8. **通知权限被拒时的诚实性（REQ-A375）**：`adb shell cmd appops set com.amos.ai POST_NOTIFICATION deny`（或 `pm revoke com.amos.ai android.permission.POST_NOTIFICATIONS`）⇒ 到点 `logcat` 应为 `notify-denied` ✓ 且**闹钟仍会响** ✓（应用在前台时 WebView 的响铃路径覆盖 ✓）；恢复后应变成 `posted` ✓。
