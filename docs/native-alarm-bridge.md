...
3. **设备侧绑定（REQ-A369 已接线）**：`crates/amos-tauri/src/alarm_sched.rs` 的 `#[cfg(feature = "android")] mod android` 持有 `JavaVM` + `Context` 的 `GlobalRef`（由 Kotlin `AlarmGlue.bind` 在 `MainActivity.onStart` 上呼 `Java_com_amos_ai_glue_AlarmGlue_attachContext` 交进来），两个命令各自调用 `AlarmGlue.schedule(ctx, id, atMs)` / `AlarmGlue.cancel(ctx, id)`（`setExactAndAllowWhileIdle(RTC_WAKEUP, …)`）。到点由 `AlarmReceiver`（`AndroidManifest.components.xml`）拉起 System UI 前台，WebView 既有的 `osAlarmWatcher` → `syncDueAlarmAlerts` 显示响铃动画。
   ...
   - **响铃那一刻的边界（生产上仍未闭合，F-TAU-012）**：`AlarmReceiver` 会"请"System UI 到前台，但**广播接收器属于后台**，而 Android 10+ 禁止后台应用启动 Activity ⇒ 平台会**丢弃并只留一条日志** ✗（本仓在 API 34 上已为「来电筛选角色对话框」实测过同一规则 ✓）。所以：**闹钟一定会触发、进程会被拉起** ✓（在进程内的 ledger/WebView 通知路径随后照常响铃 ✓），但"应用被彻底杀掉时把响铃界面**顶到屏幕最前**"这条路要按生产做法补上：**全屏 Intent 通知**（`NotificationChannel` + `setFullScreenIntent` + `USE_FULL_SCREEN_INTENT` + 运行时 `POST_NOTIFICATIONS`）——上一轮"故意不声明 `POST_NOTIFICATIONS`"的判断在那个路径落地时要一并改 ✓。真机验收时要**量**这一条：灭屏 + 杀掉应用 ⇒ 到点后看 `logcat` 是否有 `Background activity start ... blocked`（这就是该限制在本 ROM 上的证据）✓，而不只看"闹钟响了没有" ✗。
...
6. **后台启动限制（F-TAU-012，本轮新增的必测项）**：`adb shell am force-stop com.amos.ai` + 灭屏之后等到点 ⇒ 看 `logcat` 是否有 `Background activity start ... blocked` ✗ —— 有 ⇒ "顶到最前"这条路在本 ROM 上被平台挡住 ✓（正是全屏 Intent 通知要解决的问题 ✓）；**同时**确认进程被拉起且在 WebView 起来后响铃 ✓（两条要分开记 ✓）。
