...
| REQ-DC22 | 卸载策略/权限审阅经桥一致执行（关键包拒绝、未知资源拒绝） | `devcare::{apps,permission_review}` | `the_preview_and_the_enforcement_agree_for_every_package`、`uninstall_policy_holds_for_a_pinned_package_not_in_the_registry`、`the_permission_review_is_shaped_from_daemon_rows`；领域 `permissions::tests::groups_granted_resources_by_app_and_ignores_denied`、`unattributable_grants_are_dropped` |
...
| REQ-DC58 | 扫描**可复现且非递归**：子项排序使扫描顺序与文件系统无关（顺序决定哪个条目触发 `MAX_JUNK_ITEMS` 的**拒绝**），遍历改用**显式帧栈**（每条活路径一帧、前序 DFS、深度仍由 `MAX_SCAN_DEPTH` 封顶）满足 NASA Power of 10 #1 禁递归 | `hostfs::{HostFsScanner::walk, Frame::open}`、`hostfs::MAX_SCAN_DEPTH` | `the_walk_visits_in_a_reproducible_pre_order`、`scan_is_depth_bounded`、`scripts/rust-recursion-scan.mjs`（全仓门，基线为**空**） |
