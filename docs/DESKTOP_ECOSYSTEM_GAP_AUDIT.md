...
| **G6** | 桌面壳层的观感项 | **部分收口（REQ-A261 + REQ-A262 + REQ-A263 + REQ-A273 + REQ-A414 + REQ-A415 + REQ-A416）**：壳 chrome 已按 **容器 ↔ 挂件** 拆分，**五个槽位各有容器**（顶栏左右 / 舞台 / dock / 浮层），外观收进 `lib/shellChrome.ts` 一处（命中区、hover、**focus-visible 环**、读数 `tabular-nums`、dock 瓦片 / 运行点 / 菜单面板）；**浮层清单本身就是快捷键表**（F4 / ⌘Space / F3 / ⌘Tab 从注册表匹配），**层的顺序按打开顺序**（不再是组件写死的 z-index）；**系统快捷键 ⌘W/⌘M/⌘H/⌘, 作用于焦点窗口**（REQ-A273：`focusedWindowLabel` 由 `wm_windows.focused` 字段读出，5 s 心跳与 Dock 运行点共用；`label !== "main"` 且无焦点时 no-op；严格修饰键防止 `w` 被当成 `⌘W`）；**Dock 右键菜单**已落地（macOS 式下拉：`显示 / 隐藏 / 退出` 接到宿主的 `wm_focus` / `wm_hide` / `wm_close`，`选项…` 灰项 + 原因——per-app 选项菜单是缺口，不是装饰）。**控制中心已落地**（REQ-A263）：顶栏 ⚙️ 从"灰按钮"变回真开关 → macOS 式玻璃弹出面板（Wi-Fi / 蓝牙 / 飞行模式 + 深色 + 勿扰），规则与通知中心、设置屏**共用** `lib/quickRadio.ts` 一处实现（命令是端口，17 例覆盖每条设备分支）。顺带修掉的真缺陷：**同一套设备规则两份实现**（F-SH-005）、**开关型控件只做了一半**（F-SH-006：能开不能关 / 状态不说）、**层序不是打开顺序**（F-SH-007）、**系统快捷键漏到 "main" 上 ⇒ 整个桌面壳消失**（F-SH-008）、**Dock 右键菜单在卸载后访问 `$props.label` ⇒ TypeError**（F-SH-009：Svelte 5 `{#if}` + `$props` + 异步动作的必踩坑；每个菜单动作先把 `label` 读到局部再 `onclose?.()`，写在文件头 *Prop-read ordering* 段）、顶栏「控制中心」+ 主菜单五个标题 + Dock 废纸篓 + 右键「新建文件夹」四处**不可用控件假装可用**（F-SH-001）、`lib/shellChrome.ts` 声称 F4 已绑定而实际未绑定（F-SH-003）、`desktop:placeholder` 这条通道的目的端是空处理器（F-SH-004）、桌面图标几何是最后一个在模板里算几何的界面（已搬进 `lib/desktopLayout.ts`，**数值未改**）。**REQ-A414/A415/A416 补上的那一批**：Dock 两个右键菜单的"点外部关闭"（此前对着**兄弟节点**用 `dockEl.querySelector` ⇒ 两条分支恒假）、热点角落的默认动作（`toggleOverlay("spaces")` —— 注册表里没有这个 id，于是默认热区**什么都不开、还吞一次 `Escape`**）、侧边 Dock 的几何/隐藏轴/放大镜轴/自动隐藏唤起/容量轴（此前只改了外层定位类，是"位置切换只做了一半"）、Dock 溢出 `+N` 的「完整列表」（此前是 `<span title>`）、热区「显示桌面」（此前是 `console.log` TODO）、Window 菜单 Zoom / Enter Full Screen（此前是**显式空实现**）、以及**桌面形态下"应用窗口"从未画出过应用**（`Shell.svelte` 桌面分支过去无条件渲染 `DesktopShell`，于是 PC 上每个 `wm_open` 出来的窗口都是另一份桌面；现在 `s.kind === "app"` 的窗口走新 `DesktopAppWindow.svelte`）。**REQ-A417/A426/A429/A430/A431 补上的那一批**：桌面通知中心（顶栏时钟 → 右侧面板）、Dock 右键「打开」（未运行时整张菜单此前全灰）、**⌘W 终于真的关窗**（`wm_close` 此前从未真正关闭平台窗口：`close_core` 先删了 `label ⇄ id` 映射，而 `apply` 的 `Closed` 分支正是靠它解析窗口 ⇒ 命令回"成功"、窗口留在屏幕上；现在解析在状态半场**之前**、失败上报、前端新增 `host-refused` 死键原因；真机 `⌘, → ⌘W` 2 窗→1 窗）、**焦点变化不再把应用窗口变回桌面**（壳级导航观察器在每个窗口都跑 ⇒ 桌面应用窗口会把自己 `goHome()`，实测；新增 `lib/shellNav.ts` 一处判据 + **触发时**判定）、**新建窗口头几秒的 ⌘W 不再落空**（实测 3 秒序列 3/6 → 6/6：前端 `lib/earlySystemKeys.ts` 从第一帧接管这些键并在挂载后交接，菜单侧 `close_target_label` 加窗口管理器兜底，`set_focus()` 改为读回上报）。**仍未做**：macOS 红绿灯 inset ✅ **已做（REQ-A440）**——真机量出来的是"**不必留 28 px 的空白，而要把系统已经画的东西还回去**"：Overlay 模式下 AppKit 把真红绿灯画在同一条 28 px 带（与内容重叠），所以自绘那组删掉、左侧留等宽 `APP_WINDOW_TITLEBAR_INSET`；同一轮还修掉**窗口拖不动**（本树 `data-tauri-drag-region` 零处 + ACL 未授 `start_dragging` ⇒ 真机拖 24 步纹丝不动）与**标题画两遍**（`hidden_title`）；侧栏 Dock 的 padding/圆角等纯观感项。仍属缺口/设备/肉眼项：**控制中心没有**屏幕亮度与音量滑杆（本仓无亮度后端、无系统音量通路——不摆装饰性滑杆）、没有 Wi-Fi 网络选择 / 蓝牙设备列表 / 输出切换（弹出菜单）；**桌面通知中心** ✅ **已做（REQ-A417）**：`DesktopNotificationCenter.svelte` + `shellModules` 的 `notifications` 浮层，入口是**顶栏时钟**（macOS 语义），复用 `amos.notifications` store 与 `nc.*` 键；**Dock 右键菜单的"打开"项** ✅ **已做（REQ-A426）**：未运行的图标右键现在第一行是「打开」（`dock-ctx-open` → `wm_open`），其余行灰着但 `title` 说明了原因——此前整张菜单**每一行都是灰的**（没有任何一行能做任何事）；**键盘导航** ✅ **已做（REQ-A436 + REQ-A438 收尾）**：`lib/menuKeys.ts` 一处实现（上下键环绕 / Home / End / 打开即聚焦第一行 / Escape 只关本菜单 / 跳过禁用行），八处弹出菜单（三个 Dock 菜单 + 桌面右键 + 顶栏六个面板 + Apple 菜单 + 企业面板的导出下拉）全部接入，并删掉了 `DockGlobalContextMenu` 自带的私有实现；**Edit/Window 菜单的行文案随 UI 语言** ✅ **已做（REQ-A439）**（预定义项此前由 AppKit 按**系统语言**命名 ⇒ 中文 UI 里读 `Undo / Select All`、`Minimize / Zoom`；现在六行 Edit + 两行 Window 走 `*_with_text(l.<字段>)`），**⌘A（全选）的键盘通路是实测缺口**（菜单项已注册该加速键却不触发、页面也收不到键；同机同脚本 TextEdit 正常）——点击 `编辑 ▸ 全选` 可用，⌘A 不可用，见 `docs/mac-menu.md` 的实测表；per-app 菜单模型（`docs/multi-window.md` §6.5）、顶栏玻璃感、**Dock 磁吸曲线手感**（曲线仍是线性）、F3/F4 是否被 WebView / 宿主 OS 先截获（macOS 真机上 `⌘H` 走系统"隐藏应用"）、桌面状态行去留、多窗口真机 e2e | 逐项肉眼/真机复核（每项现在对应**一个挂件文件**，可独立交付） | 小 |
