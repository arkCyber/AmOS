...
| REQ-A28 | 跨系统边界全局剪贴板同步（宿主 Webview ↔ Android/Waydroid 容器文本）——宿主半 + guest 侧代理 + 通道生命周期 | `crates/amos-tauri/src/clipboard.rs`（内核 seam）、`src/clipboard_guest.rs`（宿主传输）；`crates/amos-clipboard`（`proto.rs` 单源共享协议/分帧/echo guard、`provider.rs` ClipboardProvider seam+Mock、`agent.rs` GuestAgent、`android.rs` android-gated、`link.rs` Backoff+supervise 重连）；设计 `docs/clipboard-container-sync.md` | 宿主 `clipboard_guest` **11/11**；`cargo test -p amos-tauri --lib` 143/143；`cargo test -p amos-clipboard` **43/43**（proto 编解码/echo guard、Mock provider、GuestAgent 9、link Backoff 6 + supervise 6）；`cargo test -p amos-tauri --test clipboard_e2e` **4/4**（内存全双工 host↔guest 互通无回环）；`cargo check --features android` ✓ + clippy(all-targets 默认 & android) 干净 | 🟡（P0+P1+P2a 已实现+测试；真实宿主↔guest 通道桥 P2b、接线/审计/进程托管 P3 待设备侧，见 `docs/clipboard-container-sync.md` §7） |
