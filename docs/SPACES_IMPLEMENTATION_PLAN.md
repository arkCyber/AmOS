...
>
> **状态（2026-09-19，REQ-A446）：窗口那一半也上线了。** `spaces_switch` 现在会应用
> `SpaceManager::switch_plan()` —— 切桌面时**隐藏**目标桌面不拥有的窗口、**显示**它拥有的窗口，
> 每个窗口操作失败都会 `warn!` 点名（"账本动了、屏幕没动"是用户看得见、别人看不见的状态，不能吞）。
> `active_space_windows` / `is_window_in_active_space` 这两个 helper 正是那个 plan 的组成部分，
> 因此它们从 `docs/rust-unwired-audit.md` 的豁免清单里**移出**（26 → 24），而不是被继续豁免。
>
> 三条语义（写下来是因为一份标签列表说不清，且"诚实"的那条并不"方便"）：
> **①** 没有归属任何桌面的窗口属于**所有**桌面、永不被切走 —— 唯一让窗口加入桌面的入口是显式的
> `spaces_move_window`，所以未归类不是疏漏而是"还没归档"，把未归类的窗口一起隐藏会在功能第一次
> 打开的瞬间把桌面清空；**②** Launcher（`wm::is_screen_window`）既不被隐藏也不被聚焦 —— 它就是桌面
> 本身，藏了用户无处可点，聚焦了会从用户刚回来的应用手里抢走键盘；**③** 被同时登记在两个桌面的窗口
> 由目标桌面拥有，`hide` 与 `show` 永不出现同一个标签。
>
> **状态（2026-09-19，REQ-A449）：窗口归档有 UI 了 —— 此前只有命令、没有入口。**
> `spaces_move_window` 一直在桥上（`lib/spaces.ts` 导出、Rust 命令注册、单测都过），但**没有任何
> 调用者**（记在 `frontend-ts/scripts/unwired-baseline.json` 里）⇒ 只有手工从 shell 归档过的窗口
> 才会跟着桌面切换走，而面板上没有任何地方能归档窗口。现在 `SpacesPanel` 有「窗口归档」一节：
> 列出 `wm_windows` 里**可归档**的窗口（`fileableWindows` —— 只收 `kind === "App"` 且非 `external`；
> Launcher 是桌面本身、System 表面不属于任何桌面、外部合成面没有我们的 `WebviewWindow`），每个窗口
> 一个下拉，指派到某个桌面。
>
> 归档之后**重新应用当前桌面**（`switchSpace(currentIndex)`），而不是在面板里自己判断可见性 ——
> 可见性的唯一规则是宿主的 `switch_plan`，那一行就是"让屏幕跟上账本"：刚被指派到**别的**桌面的窗口
> 会立刻离开屏幕。
>
> **状态（2026-09-19，REQ-A450）：取消归档也做了。** 上一轮登记的那条缺口 —— "未归档"是每个窗口的
> **初始状态**却**不可达**（`move_window_to_space` 只能在桌面之间搬）⇒ 点错一次就只能搬到别的桌面，
> 唯一退路是删掉那个桌面 —— 现在有反向命令：`spaces_unfile_window`（host 侧
> `SpaceManager::unfile_window()` **幂等**地把标签从**所有**桌面移出，账本没变时连平台调用都不发）
> ＋ 面板里「未归档」从 `disabled` 占位变成**真动作**。
> 有一件事 `switch_plan` 表达不了：取消归档后窗口"属于**每个**桌面"，而计划只知道**目标桌面拥有什么**
> ⇒ 窗口不在任何一份名单里，所以命令**显式把它显示出来**（它必须**现在**可见），失败则 `warn!` 点名。
>
> `space_for_window`（窗口 → 所属桌面，反方向）依然没有调用者：面板现在直接读账本（
> `spaceOfWindow`），那条反查的用处是"这个窗口住在工作桌面"这类文案，仍不存在。
> **本轮未做**：真机复跑（真窗口在真桌面上搬动需要 `make app-open`），证据是单测与类型/参数门禁。
...
- ✅ 多个独立的虚拟桌面
- ✅ 每个桌面有独立的窗口集合（**REQ-A446 起为真**：切换会通过 `switch_plan` 真的搬动窗口；此前只有账本，窗口不动）
- ✅ 桌面间快速切换（Ctrl+← / Ctrl+→）
- ✅ Mission Control 显示所有桌面
- ✅ 窗口可在桌面间移动（`spaces_move_window` 改账本；REQ-A446 起同一次切换也会让屏幕跟上）
- ✅ 桌面创建/删除/重命名
