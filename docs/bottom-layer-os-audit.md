...
| 判据项 | 代码证据 | 状态 |
|---|---|---|
| Wi-Fi / 蓝牙 / 热点(AP) | `crates/amos-radio/src/android.rs` `AndroidRadioProvider`：经 JNI `getSystemService("wifi"/"bluetooth")` → `WifiManager#setWifiEnabled/isWifiEnabled`、`BluetoothAdapter#enable/disable/isEnabled`。**只有开/关与查询**；无 `ConnectivityManager` 现代化路径、无扫描/选网/SSID 列表、无蓝牙扫描/配对。飞行模式是 AmOS 侧位，未写 `Settings.Global.AIRPLANE_MODE_ON`。**热点（AP / 网络共享）**已接真栈：`TetheringGlue.kt`（`android-glue/`）+ `TetheringManager#startTethering(TetheringRequest, …)`/`#stopTethering`；**公开 tethering API 自 API 36 才有**（`javap` 核实：android-34/35 的 `android.jar` 无 `TetheringManager`；`ConnectivityManager` 的 tethering 成员是 `@SystemApi`）⇒ 36 以下 glue 如实返回 `false`；另需 `TETHER_PRIVILEGED`（signature\|privileged）。状态由 `TetheringEventCallback` 实时维护（含 AmOS 之外的开关）。AP 的 SSID/口令/频段：**平台限制（非待办）** —— 公开的 `SoftApConfiguration.Builder` 只有 `setChannels`（`setSsid`/`setPassphrase`/`setBand`/`setSecurityType` 是 `@SystemApi`）⇒ 非特权安装无法构造带 SSID/口令的配置，`TetheringRequest.Builder#setSoftApConfiguration` 因此对普通应用不可用（`javap` 核实，见 `docs/radio.md` §7）。设置页配置只作**用户意图**持久化 | 🟠 编译门控骨架，真机未接通；Wi-Fi/蓝牙仅"总开关"级；热点已接真 API（Kotlin 由 `make android-glue-check` 真实 SDK 编译门覆盖）但受 API 36 + `TETHER_PRIVILEGED` + `SoftApConfiguration` 三重平台限制 |
...
2. **[桥] Radio 真机接通 & 功能深化**：`AndroidRadioProvider` 尚未在 System UI 用真 `Context` 替换 Mock；Wi-Fi 升 `ConnectivityManager`、蓝牙加 `BLUETOOTH_CONNECT` 权限、飞行写 `Settings.Global`（`docs/radio.md` §6）。当前产品只有"开关"，无选网/配对/状态枚举。**热点（AP）**已接真 tethering 调用（`TetheringGlue.kt`，`docs/radio.md` §7），Kotlin 侧由 `make android-glue-check` 用真实 SDK 编译覆盖；但受 **API 36（公开 tethering API 的下限）+ `TETHER_PRIVILEGED`（signature\|privileged）**双重限制：36 以下或非特权安装会被如实拒绝（报错而非谎报）。AP 的 SSID/口令配置通路（`TetheringRequest.Builder#setSoftApConfiguration`）**对非特权应用不可用**（公开的 `SoftApConfiguration.Builder` 只有 `setChannels`，其余 setter 是 `@SystemApi`），故这不是我方待办而是平台限制 —— 见 `docs/radio.md` §7。
